...
class Indicator:
    ...
    def rsi(self, peroid: int, method: str = 'wilders') -> pd.DataFrame:
        locals_data = locals()
        del locals_data['self']
        ...
        self._frame['ewma_up'] = self._price_groups['up_day'].transform(
            lambda x: x.ewm(span=peroid).mean()
        )
    ...
    def sma(self, peroid: int) -> pd.DataFrame:
        locals_data = locals()
        del locals_data['self']
        ...
        self._frame[column_name] = self._price_groups['close'].transform(
            lambda x: x.rolling(window=peroid).mean()
        )

        return self._frame

    def ema(self, peroid: int, alpha: float = 0.0) -> pd.DataFrame:
        locals_data = locals()
        del locals_data['self']
